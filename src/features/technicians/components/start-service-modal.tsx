      ...
      <div className="flex max-h-[98vh] w-full max-w-4xl flex-col overflow-hidden rounded-2xl bg-white shadow-2xl sm:max-h-[95vh]">
        ...
        <header className="flex flex-col items-start gap-2 border-b border-slate-200 px-4 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-8 sm:py-6">
          <div>
            ...
            <p className="mt-0.5 text-sm text-slate-500 sm:mt-1">
              Confirma tu llegada y registra las fotografías del estado inicial.
            </p>
          </div>
          <button
            type="button"
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 sm:px-8 sm:py-6">
          <div className="space-y-6 sm:space-y-8">
            ...
            <section className="grid gap-3 rounded-xl bg-slate-50 p-4 sm:grid-cols-2 sm:gap-5 sm:p-6">
              <InfoRow label="Empresa" value={service.client.companyName} />
              <InfoRow label="Contacto" value={service.client.contactFirstName || "-"} />
              <InfoRow label="Dirección" value={service.client.address} />
              <InfoRow label="Teléfono" value={service.client.phone} />
              <InfoRow
                label="Fecha"
                value={new Date(service.scheduledAt).toLocaleString("es-CO")}
              />
              <InfoRow label="Estado" value={service.status} />
            </section>
              ...
              <div className="flex flex-col items-start gap-1 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  ...
                  <p className="text-sm text-slate-500">
                    Captura el estado inicial antes de comenzar el trabajo.
                  </p>
                </div>
                <span className="rounded-full bg-green-100 px-3 py-0.5 text-sm font-medium text-green-700 sm:px-4 sm:py-1">
                  {files.length} fotografía{files.length !== 1 && "s"}
                </span>
              </div>

              <label
                htmlFor="before-images"
                className="flex min-h-[180px] cursor-pointer flex-col items-center justify-center rounded-xl border-2 border-dashed border-slate-300 p-4 text-center transition hover:border-green-700 hover:bg-green-50 sm:min-h-[200px] sm:p-6"
              >
                <ImagePlus size={40} className="mb-2 text-green-700 sm:mb-3 sm:size-12" />
                    ...
                    >
                      <div className="relative aspect-square">
                        ...
                        <button
                          ...
                          className="absolute right-1.5 top-1.5 rounded-full bg-red-600 p-1.5 text-white transition hover:bg-red-700 sm:right-2 sm:top-2 sm:p-2"
        ...
        <footer className="flex flex-col-reverse gap-3 border-t border-slate-200 px-4 py-4 sm:flex-row sm:items-center sm:justify-end sm:gap-4 sm:px-8 sm:py-6">
          <button
            ...
            disabled={loading}
            onClick={onClose}
            className="w-full rounded-lg border border-slate-300 px-4 py-2.5 text-sm transition hover:bg-slate-100 sm:w-auto sm:px-6 sm:py-2"
          >
            Cancelar
          </button>
          <button
          ...
          >
            ...
            {loading ? "Subiendo fotografías..." : "Iniciar servicio"}
          </button>
        </footer>
      </div>
    </div>
...
function InfoRow({ label, value }: InfoRowProps) {
  return (
    <div>
      ...
      <p className="mt-0.5 font-medium text-slate-900 sm:mt-1">{value}</p>
    </div>
  );
}